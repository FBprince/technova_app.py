...
def copy_button(text: str, label: str = "Copy", key: str = None):
    ...
    <button id="{el_id}" onclick="navigator.clipboard.writeText(atob('{safe_b64}'))"
        style="background:transparent;border:1px solid rgba(255,255,255,0.08);
               color:#EAF9FF;padding:8px 14px;border-radius:10px;cursor:pointer;
               font-weight:700;margin-right:8px;">
      📋 {label}
    </button>
    <script>
...
def set_canvas_background():
    canvas_html = """
    <canvas id="tech-canvas" style="position:fixed; top:0; left:0; width:100%; height:100%; z-index:-1000;"></canvas>
    <script>
    ...
    for(let i=0;i<numNodes;i++){
      nodes.push({x:Math.random()*canvas.width,y:Math.random()*canvas.height,dx:(Math.random()-0.5)*0.6,dy:(Math.random()-0.5)*0.6,r:Math.random()*1.5+0.5});
    }
    function draw(){
      ...
      for(let i=0;i<nodes.length;i++){
        let n = nodes[i];
        ctx.beginPath(); ctx.arc(n.x,n.y,n.r,0,Math.PI*2); ctx.fillStyle='rgba(0,255,255,0.8)'; ctx.fill();
        n.x+=n.dx; n.y+=n.dy;
        ...
        for(let j=i+1;j<nodes.length;j++){
          ...
          if(d<120){ ctx.beginPath(); ctx.moveTo(n.x,n.y); ctx.lineTo(nodes[j].x,nodes[j].y); ctx.stroke(); }
        }
      }
      requestAnimationFrame(draw);
    }
    ...
    <style>
    body, [data-testid="stAppViewContainer"] {
      background-color:#0d0d1a; color:#00f9ff;
      font-family: 'Orbitron', 'Trebuchet MS', monospace;
      text-shadow:0 0 5px #00f9ff,0 0 10px #00cfff; overflow-x:hidden;
    }
    .stButton>button, .stDownloadButton>button { 
        background: linear-gradient(90deg,#00f0ff,#0066ff); 
        color:#02121b; border-radius:12px; padding:8px 16px; font-weight:700; 
        transition: transform 0.2s;
    }
    .stButton>button:hover, .stDownloadButton>button:hover { transform: scale(1.05); }
    .stTextInput>div>input, .stTextArea>div>textarea, .stSlider>div>div>input {
        background:#0d0d1a; color:#00f9ff; border:1px solid #00f9ff; border-radius:8px;
    }
    .stCheckbox>div>label { color:#00f9ff; }
    .stExpanderHeader { background-color: rgba(0,255,255,0.05); border-radius:8px; padding:6px 10px; }
    </style>
    """
    st.markdown(canvas_html, unsafe_allow_html=True)
