...
# technova_app.py (Fully Combined Neon-Mature UI + All Tabs)
import streamlit as st
import requests
from bs4 import BeautifulSoup
import re
...
# ----------------------------
# Neon-mature styling helpers
# ----------------------------
def styled_header(text):
    st.markdown(f"""
    <h2 style='color: #00f9ff; text-shadow:0 0 6px #00f9ff,0 0 12px #00cfff; font-family: "Orbitron", "Trebuchet MS", monospace; margin-bottom:0.2em;'>{text}</h2>
    """, unsafe_allow_html=True)

def styled_caption(text):
    st.markdown(f"""
    <p style='color: #a0f0ff; font-family: "Trebuchet MS", monospace; font-size:0.9em; margin-top:0.0em;'>{text}</p>
    """, unsafe_allow_html=True)

def neon_box(text):
    st.markdown(f"""
    <div style='border:1px solid #00f9ff; border-radius:8px; padding:10px; background-color: rgba(10,11,28,0.5); color:#e0f9ff; font-family: "Trebuchet MS", monospace;'>{text}</div>
    """, unsafe_allow_html=True)
...
def copy_button(text: str, label: str = "Copy", key: str = None):
    ...
    html = f"""
    <button id="{el_id}" onclick="navigator.clipboard.writeText(atob('{safe_b64}'))"
        style="background:transparent;border:1px solid rgba(255,255,255,0.06); color:#EAF9FF;padding:8px 12px;border-radius:8px;cursor:pointer;font-weight:700;margin-right:8px;">
      📋 {label}
    </button>
    <script>
    const btn_{el_id} = document.getElementById('{el_id}');
...
.stButton>button, .stDownloadButton>button { background: linear-gradient(90deg,#00e0ff,#0044cc); color:#f0f0f0; border-radius:8px; padding:8px 12px; font-weight:600; transition: transform 0.15s; }
.stButton>button:hover, .stDownloadButton>button:hover { transform: scale(1.03); }
</style>
"""
st.markdown(canvas_html, unsafe_allow_html=True)
...
# App title & caption
# ----------------------------
styled_header("🌌 Technova AI Toolkit")
styled_caption("Document summarizer • Code analyzer • AI vs Human scanner • URL fetcher — neon UI")

# ----------------------------
# Utility Functions: Summarizer, Code Analyzer, AI Detector, URL Fetcher
# ----------------------------
...
# ----------------------------
# Tabs logic will be implemented next (Document Summarizer done above). Code Analyzer, AI Scanner, URL Fetch to be completed fully.
# ----------------------------
