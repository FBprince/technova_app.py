...
#     st.markdown('<div class="tab-content">', unsafe_allow_html=True)
#     st.subheader("📄 Text Summarizer & Q&A")
#     st.write("Summarize long texts, articles, or documents into concise summaries. Ask questions about your content!")
...
#             with st.spinner("Extracting text from PDF..."):
#                 text_to_summarize = extract_text_from_pdf(uploaded_file)
#                 content_source = f"PDF document ({uploaded_file.name})"
#                 if text_to_summarize and not text_to_summarize.startswith("Error"):
#                     st.success(f"Extracted {len(text_to_summarize)} characters from PDF")
#                 else:
#                     st.error(text_to_summarize)
...
#             with st.spinner("Scraping website content..."):
#                 text_to_summarize = scrape_website(url)
#                 content_source = f"website ({get_website_title(url)})"
#                 if text_to_summarize and not text_to_summarize.startswith("Error"):
#                     st.success(f"Scraped {len(text_to_summarize)} characters from website")
#                     st.text_area("Scraped content preview:", text_to_summarize[:500] + "...", height=100)
#                 else:
...
#     if text_to_summarize and not text_to_summarize.startswith("Error"):
#         st.session_state.summarizer_content = text_to_summarize
#         st.session_state.summarizer_source = content_source
...
#     # Q&A Section
#     if hasattr(st.session_state, 'summarizer_content') and st.session_state.summarizer_content:
#         st.markdown("---")
#         st.markdown('<div class="qa-section">', unsafe_allow_html=True)
#         st.subheader("❓ Ask Questions About Your Content")
#         st.write(f"Ask questions about your {st.session_state.summarizer_source}. The AI will answer based on the content, or provide general knowledge if the question isn't directly related.")
        
#         question = st.text_input("Enter your question:", 
#                                 placeholder=f"What would you like to know about your {st.session_state.summarizer_source}?",
#                                 key="summarizer_question")
        
#         if st.button("Get Answer", use_container_width=True, key="summarizer_qa") and question:
...
#                     st.write(answer)
#                     st.markdown('</div>', unsafe_allow_html=True)
#                     copy_button(answer, "Copy Answer", "summarizer_answer")
#                 else:
#                     st.error("Failed to generate answer. Please try again.")
        
#         # Clear content button
#         if st.button("🗑️ Clear Content", key="clear_summarizer_content"):
#             if 'summarizer_content' in st.session_state:
#                 del st.session_state.summarizer_content
#             if 'summarizer_source' in st.session_state:
#                 del st.session_state.summarizer_source
#             st.rerun()
...
# def web_fetcher_tab():
#     """Web Fetcher, Summarizer & Q&A Tool"""
#     st.markdown('<div class="tab-content">', unsafe_allow_html=True)
#     st.subheader("🌐 Web Fetcher, Summarizer & Q&A")
#     st.write("Enter any website or article link, and our AI will instantly fetch the content, clean it up, and generate a clear summary for you. Ask questions about the webpage content!")
    
#     if not check_tab_access("Web Fetcher"):
#         st.markdown('</div>', unsafe_allow_html=True)
#         return
...
#                        help="Paste any website or article URL")
    
#     # Summary options
#     col1, col2 = st.columns(2)
#     with col1:
#         summary_length = st.selectbox("Summary length:", ["Short", "Medium", "Detailed"], key="web_summary_length")
#     with col2:
#         summary_focus = st.selectbox("Summary focus:", ["General Overview", "Key Points", "Main Arguments", "Facts & Data"], key="web_summary_focus")
    
#     # Fetch and process button
#     if st.button("🚀 Fetch & Summarize", use_container_width=True) and url:
...
#                 st.session_state.web_content = website_content
#                 st.session_state.web_url = url
#                 st.session_state.web_title = website_title
                
#                 # Show content info
#                 st.info(f"📄 Fetched content from: **{website_title}**\nContent length: {len(website_content)} characters")
...
#                     focus_map = {
#                         "General Overview": "providing a general overview",
#                         "Key Points": "highlighting the key points and main ideas",
#                         "Main Arguments": "focusing on the main arguments and conclusions",
#                         "Facts & Data": "emphasizing important facts, data, and statistics"
#                     }
                    
#                     system_prompt = "You are an expert content summarizer. Create clear, informative summaries that capture the essence of web content while being engaging and easy to understand."
                    
#                     prompt = f"Summarize the following webpage content {length_map[summary_length]} {focus_map[summary_focus]}:\n\nWebsite: {website_title}\nURL: {url}\n\nContent:\n{website_content[:5000]}"
...
#     # Q&A Section for fetched content
#     if hasattr(st.session_state, 'web_content') and st.session_state.web_content:
#         st.markdown("---")
#         st.markdown('<div class="qa-section">', unsafe_allow_html=True)
#         st.subheader("❓ Ask Questions About This Webpage")
#         st.write(f"Ask questions about **{st.session_state.web_title}**. The AI will answer based on the webpage content, or provide general knowledge if the question isn't directly related.")
        
#         # Show current webpage info
#         with st.expander("ℹ️ Current Webpage Info"):
...
#         with col1:
#             if st.button("Get Answer", use_container_width=True, key="web_qa") and question:
...
#                         st.write(answer)
#                         st.markdown('</div>', unsafe_allow_html=True)
#                         copy_button(answer, "Copy Answer", "web_answer")
#                     else:
#                         st.error("Failed to generate answer. Please try again.")
        
#         with col2:
#             if st.button("🗑️ Clear Webpage", key="clear_web_content"):
#                 if 'web_content' in st.session_state:
#                     del st.session_state.web_content
#                 if 'web_url' in st.session_state:
#                     del st.session_state.web_url
#                 if 'web_title' in st.session_state:
#                     del st.session_state.web_title
#                 st.rerun()
...
#     # Main tabs (updated to include Web Fetcher)
#     tabs = st.tabs([
#         "📄 Text Summarizer",
#         "🌐 Web Fetcher",
#         "💻 Code Generator", 
